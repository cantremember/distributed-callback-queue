...
const {
  serializeError,
  deserializeError
} = require('serialize-error'); // callback buckets
...
const arg = args[0];
// console.log('--- queue', arg && arg.name)
  callback(...args); // clean local queue
...
exports.createPublisher = function createPublisher(redis, pubsubChannel, logger) {
  return async function publishResult(lockRedisKey, err, ...args) {
    ...
    const localArgs = [err, ...args]; // post result to other processes

    redis.publish(pubsubChannel, JSON.stringify([lockRedisKey, broadcastArgs])).catch(e => {
      logger.warn({
        err: e
...
exports.createConsumer = function createConsumer(redis, pubsubChannel, logger) {
  const connect = () => redis.subscribe(pubsubChannel).tap(() => {
