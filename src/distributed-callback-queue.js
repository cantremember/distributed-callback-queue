...
const assert = require('assert'); // may only use redis with bluebird promise


Redis.Promise = Promise; // lodash helpers
...
const compose = require('lodash/fp/compose'); // internal deps
...
class DistributedCallbackQueue {
  constructor(options = {}) {
    ...
    const pubsub = options.pubsub || (typeof client.duplicate === 'function' ? client.duplicate({
      lazyConnect: false
    }) : client);
    ...
    const logger = this.logger = DistributedCallbackQueue.initLogger(options); // put on the instance
    ...
    pubsub.on('messageBuffer', this.consume); // ready
  ...
  async push(suffix, next, timeout = this.lockOptions.timeout * 2) {
    assert(suffix, 'must be a truthy string'); // first queue locally to make use of pending requests

    const lockRedisKey = this.key(suffix);
    const queued = callbackQueue.add(lockRedisKey, next); // this means that we already have local callback queue with that
    // identifier, don't try to lock it again and proceed further

    if (!queued) {
// console.warn('--- push couldNotAcquireLockError');
    ...
    const lock = this.getLock(); // get the lock
...
// console.warn('--- push publish', e.name);
        await this.publish(lockRedisKey, e);
      }

// console.warn('--- push throw', e.name);
      throw e;
    }
    finally {
// console.warn('--- lock C')
  ...
  async fanout(suffix, ...props) {
    ...
    let jobAbortReject;
    let jobAbortPromise = new Promise((_resolve, reject) => {
    ...
    const promise = new Promise((resolve, reject) => {
      onJobCompleted = (err, ...args) => {
        if (err) {
// console.warn('--- onJobCompleted err', err.name);
          if (jobAbortPromise) {
            jobAbortReject(err);
          }
          setImmediate(() => {
...
// console.warn('--- onJobCompleted ...');
    try {
      // bluebird Promise for "cancelling"
      pushPromise = this.push(suffix, onJobCompleted, timeout);
      onCompleted = await Promise.race([ pushPromise, jobAbortPromise ]);
// console.warn('--- onJobCompleted : pushed');
      jobAbortPromise = undefined;
    } catch (err) {
// console.warn('--- onJobCompleted push#catch', err.name);
      if (notLockAcquisitionError(err)) {
// console.warn('--- onJobCompleted => A');
        setImmediate(onJobCompleted, err);
        return promise;
      }

// console.warn('--- onJobCompleted => B');
      return promise;
    ...
    try {
      const result = await Promise.race([performWork, promise]);
// console.warn('--- onJobCompleted : race');
      setImmediate(onCompleted, null, result);
    } catch (err) {
// console.warn('--- onJobCompleted performWork#catch', err.name);
...
// console.warn('--- onJobCompleted : return');
  ...
  once(suffix) {
    ...
    return lock.acquire(lockRedisKey).return(lock);
  }
  /**
  ...
  multi(...args) {
    ...
    return Promise.map(actions, action => this.once(action).reflect()).then(MultiLock.batchAction).catch(MultiLockError, MultiLock.cleanup).then(locks => new MultiLock(locks));
  }
  /**
  ...
  serial(lockKey, jobFunction) {
    const workUnit = next => {
      ...
      const done = err => {
        ...
        if (called === true) return null; // increase counter for further queueing

        called += 1; // error handling
        ...
        if (called === 1) return null; // if we were not rejected - return control

        if (rejected === false) return next(); // try requeueing and basically repeating operation until it succeeds

        return Promise.fromCallback(workUnit).asCallback(next);
      };

      return this.push(lockKey, done).then(jobFunction).asCallback(done);
    };
  ...
  createWorker(lockRedisKey, acquiredLock) {
    ...
    const broadcastJobStatus = async (err, ...args) => {
      ...
      const {
        lock
      } = broadcastJobStatus; // because a job may take too much time, other listeners must implement timeout/retry strategy
      ...
      broadcastJobStatus.lock = null; // must release lock now. Technically there could be an event
